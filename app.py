...
def reservas_route():
    ...
    if request.method == "POST":
        ...
        flash("Erro ao adicionar a reserva. Verifique os dados e tente novamente.", "error")
        return render_template(
            "reservas.html",
            title="Reservas",
...
@app.route("/reservas/<int:id_reserva>/editar", methods=["GET"])
def editar_reserva_page(id_reserva):
    """Página de edição de uma reserva existente."""
    reserva = next((res for res in reservas.buscar_reservas_com_imoveis() if res['id'] == id_reserva), None)
    if not reserva:
        flash("Reserva não encontrada.", "error")
        return redirect(url_for("reservas_route"))
    return render_template("editar_reserva.html", title="Editar Reserva", reserva=reserva)
...
def editar_reserva(id_reserva):
    ...
    if sucesso:
        ...
        return redirect(url_for("reservas_route"))
    flash("Erro ao atualizar a reserva. Verifique os dados e tente novamente.", "error")
    return redirect(url_for("reservas_route"))
...
def imoveis_route():
    ...
    if request.method == "POST":
        ...
        flash("Erro ao adicionar o imóvel. Verifique os dados e tente novamente.", "error")
        return render_template(
            "imoveis.html",
            title="Imóveis",
            imoveis=reservas.fetch_table("imoveisbeta", order_by="numero_apto"),
